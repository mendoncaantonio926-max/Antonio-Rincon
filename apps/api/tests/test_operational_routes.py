...
def test_onboarding_billing_and_dashboard_flow() -> None:
    ...
    assert isinstance(dashboard_response.json()["window_pressure"], list)
    assert isinstance(dashboard_response.json()["rebalance_suggestions"], list)
    assert isinstance(dashboard_response.json()["owner_capacity"], list)
    ...
    assert isinstance(dashboard_response.json()["owner_stage_mix"], list)
    assert "forecast_confidence" in dashboard_response.json()
    assert "goal_risk" in dashboard_response.json()
    ...
    assert "action_label" in dashboard_response.json()["forecast_playbook"][0]
...
def test_lead_pipeline_update_and_filters() -> None:
    ...
    assert isinstance(dashboard_response.json()["forecast_blockers"], list)
    assert isinstance(dashboard_response.json()["forecast_playbook"], list)
    assert dashboard_response.json()["forecast_playbook"][0]["follow_up_at"] is not None
...
def test_membership_invite_and_ai_summary() -> None:
    ...
    assert ai_response.json()["execution_mode"] == "update_lead_batch"
